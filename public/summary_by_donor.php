...
// ---------- โหลดรายชื่อผู้นำบุญ (สำหรับ dropdown ค้นหา) ----------
$donor_list = [];
$dstmt = $conn->prepare("SELECT id, full_name FROM donors WHERE is_deleted = 0 OR is_deleted IS NULL ORDER BY full_name");
$dstmt->execute();
$dres = $dstmt->get_result();
while ($d = $dres->fetch_assoc()) { $donor_list[] = $d; }
$dstmt->close();

$donor_id = isset($_GET['donor_id']) ? intval($_GET['donor_id']) : 0;

// ---------- สรุปตามผู้นำบุญ (กราฟ + ตาราง) ----------
$rows = [];
$grand_total = 0;
$total_count = 0;
$sql = "
    SELECT dn.full_name AS label, SUM(d.amount) AS total, COUNT(*) AS cnt
...
// ---------- ถ้าเลือกผู้นำบุญ: รวมยอดตาม (ผู้นำบุญ + ประเภทบุญ) + รายละเอียด ----------
$donor_summary = [];
$donor_details = [];
if ($donor_id > 0) {
    $ds = $conn->prepare("
        ...
        WHERE d.donation_date BETWEEN ? AND ? AND d.is_deleted = 0 AND d.donor_id = ?
        GROUP BY d.donor_id, d.merit_id
        ORDER BY total DESC
    ");
    $ds->bind_param("ssi", $start_date, $end_date, $donor_id);
} else {
    $ds = $conn->prepare("
        SELECT d.donor_id, dn.full_name, d.merit_id, m.merit_name,
               SUM(d.amount) AS total, COUNT(*) AS cnt
...
// หาชื่อผู้นำบุญที่เลือก
$sel_donor_name = '';
foreach ($donor_list as $d) { if ((int)$d['id'] === $donor_id) { $sel_donor_name = $d['full_name']; break; } }
$conn->close();
...
<div class="container mt-4">
    <h3 class="mb-4"><i class="bi bi-person-heart text-primary"></i> สรุปตามผู้นำบุญ</h3>
    ...
    <!-- เลือกผู้นำบุญ (dropdown ค้นหาได้) + Excel -->
    <div class="row g-3 mb-4">
        <div class="col-md-8">
            <label class="form-label">เลือกผู้นำบุญ (เพื่อดูรายละเอียดตามประเภทบุญ)</label>
            <select id="donorSelect" class="selectpicker form-control" data-live-search="true" title="-- เลือกผู้นำบุญทั้งหมด --">
                <?php foreach ($donor_list as $d): ?>
                <option value="<?php echo $d['id']; ?>" <?php echo $donor_id===(int)$d['id']?'selected':''; ?>><?php echo htmlspecialchars($d['full_name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    ...
    <div class="card shadow-sm p-3 mb-4 text-center">
        ...
        <small class="text-muted"><?php echo number_format($total_count); ?> รายการ • <?php echo count($rows); ?> ผู้นำบุญ</small>
    </div>

    <div class="row">
        <div class="col-md-5">
            <div class="card shadow-sm p-3 mb-4">
                <h5 class="text-success mb-3">สัดส่วนตามผู้นำบุญ</h5>
                <?php if (empty($rows)): ?>
        ...
        <div class="col-md-7">
            <table class="table table-striped bg-white shadow-sm">
                <thead class="table-success">
                    <tr><th>ผู้นำบุญ</th><th class="text-end">จำนวนรายการ</th><th class="text-end">ยอดรวม (เยน)</th></tr>
                </thead>
                <tbody>
    ...
    <!-- รวมยอดตาม (ผู้นำบุญ + ประเภทบุญ) — แสดงเสมอ -->
    <div class="card shadow-sm p-3">
        <h5 class="text-primary mb-3">
            รวมยอดตามผู้นำบุญและประเภทบุญ
            <?php if ($donor_id > 0): ?>— <?php echo htmlspecialchars($sel_donor_name); ?><?php endif; ?>
...
$(function(){
    ...
    $('#donorSelect').on('changed.bs.select', function(e) {
        $('#donor_id').val($(this).val());
        $('#filterForm').submit();
    });
});
...
function updateExcelLink() {
    ...
    var did = document.getElementById('donor_id').value;
    var q = p + (did ? '&donor_id=' + encodeURIComponent(did) : '');
    document.getElementById('excelBtn').href = 'summary_by_donor_export.php?' + q;
}
updateExcelLink();

const donorLabels = <?php echo json_encode($labels, JSON_UNESCAPED_UNICODE); ?>;
const donorValues = <?php echo json_encode($values); ?>;
if (donorLabels.length > 0) {
    new Chart(document.getElementById('donorChart'), {
        type: 'pie',
        data: { labels: donorLabels, datasets: [{ data: donorValues }] },
