...
// ชื่อผู้นำบุญที่เลือก (ถ้ามี)
$sel_donor_name = 'ทุกผู้นำบุญ';
if ($donor_id > 0) {
    $ds = $conn->prepare("SELECT full_name FROM donors WHERE id = ?");
    $ds->bind_param("i", $donor_id);
    $ds->execute();
    $dr = $ds->get_result()->fetch_assoc();
    if ($dr) $sel_donor_name = $dr['full_name'];
    $ds->close();
}
...
if ($donor_id > 0) {
    $det = $conn->prepare("
        ...
        WHERE d.donor_id = ? AND d.donation_date BETWEEN ? AND ? AND d.is_deleted = 0
        ORDER BY d.donation_date ASC, d.id ASC
    ");
    $det->bind_param("iss", $donor_id, $start_date, $end_date);
} else {
    $det = $conn->prepare("
        SELECT d.donation_date, dn.full_name, m.merit_name, d.receipt_name, d.amount, d.comment
        FROM donation_items d
...
$rows = [];
$rows[] = [['s' => 'รายการบริจาคตามผู้นำบุญ: ' . $sel_donor_name . ' — ' . $period_title]];
...
$suffix = $donor_id > 0 ? 'donor' . $donor_id : 'all';
$filename = 'donations_by_donor_' . $suffix . '_' . $start_date . '_to_' . $end_date . '.xlsx';
